packagepackage SpyderCore.Listener;
...
public class PlayerListener implements Listener {
    ...
    public void onPickUpItem(InventoryPickupItemEvent event) {
        ...
        dataLog.addPlayerData(player.getName(), PluginLoader.getLocalTime() + "撿起了物品：" + event.getItem().getName());
    }
    ...
    public void onBreak(BlockBreakEvent event){
        Player player = event.getPlayer();
        dataLog.addPlayerData(player.getName(), PluginLoader.getLocalTime() + "破壞了" + event.getBlock().getName() +
                "在位置" + event.getBlock().getLocation().toString());
    }
}
